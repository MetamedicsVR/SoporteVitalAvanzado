...
public class NPCSpotManager : MonoBehaviourInstance<NPCSpotManager>
{
    ...
    {
        NPCSpot availableSpot = spots .Where(spot => spot.type == spotType && spot.available) .OrderBy(spot => Vector3.Distance(spot.transform.position, position)) .FirstOrDefault();
        if (availableSpot)
        {
